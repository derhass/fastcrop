...
/* Print the util::info log of the shader compiler/linker.
...
extern GLuint shaderCreateAndCompile(GLenum type, const GLchar *source)
{
	...
	util::info("created shader object %u",shader);
	glShaderSource(shader, 1, (const GLchar**)&source, NULL);
	util::info("compiling shader object %u",shader);
	glCompileShader(shader);

	glGetShaderiv(shader, GL_COMPILE_STATUS, &status);
	if (status != GL_TRUE) {
		util::warn("Failed to compile shader");
...
extern GLuint shaderCreateFromFileAndCompile(GLenum type, const char *filename)
{
	...
	util::info("loading shader file '%s'",filename);
	FILE *file = fopen(filename, "rt");
	if(!file) {
		util::warn("Failed to open shader file '%s'", filename);
		return 0;
	}
	...
	if (!source) {
		util::warn("Failed to allocate memory for shader file '%s'", filename);
		fclose(file);
		return 0;
	}
	...
	free(source);
	if (!shader) {
		util::warn("Failed to compile shader '%s'", filename);
	}
	return shader;
}
...
extern GLuint programCreate(GLuint vertex_shader, GLuint fragment_shader)
{
	...
	util::info("created program %u",program);
	...
	util::info("linking program %u",program);
	glLinkProgram(program);

	glGetProgramiv(program, GL_LINK_STATUS, &status);
	if (status != GL_TRUE) {
		util::warn("Failed to link program!");
...
extern GLuint programCreateFromFiles(const char *vs, const char *fs)
{
	...
	util::info("destroying shader object %u",id_vs);
	glDeleteShader(id_vs);
	util::info("destroying shader object %u",id_fs);
...
{
#ifdef WIN32
	std::wstring filename_wide = util::utf8ToWide(std::string(filename));
	std::wstring mode_wide = util::utf8ToWide(std::string(mode));
	FILE *file = NULL;
